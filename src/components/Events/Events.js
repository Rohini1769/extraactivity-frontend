...
function Events() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get('https://activitymanagement.up.railway.app/events')
      .then((response) => {
  ...
  const handleDeleteConfirmation = (confirm) => {
    if (confirm && eventToDelete) {
      axios
        .delete(`https://activitymanagement.up.railway.app/delete-event?eventName=${eventToDelete.eventName}`)
        .then(() => {
  ...
  const handleSaveEditedEvent = (e) => {
    ...
    axios
      .post('https://activitymanagement.up.railway.app/update-event', formData, {
        headers: {
  ...
  const handleViewStudents = (event) => {
    ...
    axios
      .get('https://activitymanagement.up.railway.app/students-by-event', {
