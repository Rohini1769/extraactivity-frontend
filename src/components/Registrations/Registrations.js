...
export default function RegistrationsTable() {
  ...
  useEffect(() => {
    // Fetch initial data
    axios
      .get('https://activitymanagement.up.railway.app/all-registrations')
      .then((response) => {
  ...
  const handleDeleteClick = (id) => () => {
    axios
      .delete(`https://activitymanagement.up.railway.app/unregister?email=${rows.find(row => row.id === id).studentEmail}&eventname=${rows.find(row => row.id === id).eventName}`)
      .then(() => {
