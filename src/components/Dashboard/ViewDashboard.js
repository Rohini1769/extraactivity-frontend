...
const ViewDashboard = () => {
  ...
  useEffect(() => {
    axios.get('https://activitymanagement.up.railway.app/events')
      .then((response) => {
        ...
        const registrationPromises = eventNames.map(eventName =>
          axios.get(`https://activitymanagement.up.railway.app/students-by-event?eventName=${eventName}`).then(res => res.data.length)
        );
  ...
  useEffect(() => {
    if (studentsData.length > 0) {
      const studentPromises = studentsData.map(student => 
        axios.get(`https://activitymanagement.up.railway.app/registered-events?email=${student.email}`).then(res => res.data)
      );
